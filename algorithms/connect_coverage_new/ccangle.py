...
def ccangle(agentPos, angles, ueHisY, agentAngles, angleStart, angleEnd, radius, vMax, cov):
    ...
    beta = 10000
    gamma = 0.0
    gradient = (gamma - beta) / (positionEnd - positionStart)
    intercept = beta - positionStart * gradient
    ...
    angleChangeIndex = np.abs(np.arctan(ue/vMax) - agentAngles) > np.pi / 30
    # 符合条件
    newAngle = agentAngles[angleChangeIndex] + np.sign(angles[angleChangeIndex] - bestAngle[angleChangeIndex]) * np.pi / 30
    ue[angleChangeIndex] = vMax * np.tan(newAngle)

    # 保证无人机速度范围在-pi/3 ~ pi/3
    lessIndex = np.arctan(ue / vMax) < -np.pi / 3
    largerIndex = np.arctan(ue / vMax) > np.pi / 3

    ue[lessIndex] = vMax * np.tan(-np.pi / 3)
    ue[largerIndex] = vMax * np.tan(np.pi / 3)
    return ue
