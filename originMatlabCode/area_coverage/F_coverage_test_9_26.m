function [] = F_coverage_test_9_26(UAVnumber,R,numIterations)
% close all;
% clear;
%默认参数：UAVnumber = 60；R = 25;numIterations = 2500
...
for i = 1:UAVnumber  %维诺划分的设置
    ...
    verCellHandle(i) = patch(v(c{i},1),v(c{i},2),'w','LineWidth',...
        1,'EdgeColor','w','FaceColor','none');
    hold on
end
%无人机轨迹
...
% for i = 1:UAVnumber
%     CoverageRangeHandle(i) = rectangle('Position',[PathX(1,i)-coverageRadius...
%         PathY(1,i)-coverageRadius 2*coverageRadius 2*coverageRadius],...
%         'Curvature',[1,1],'FaceColor','none','EdgeColor','b');
%     %CoverageRangeHandle(i) = viscircles([PathX(1,i) PathY(1,i)],
%     %coverageRadius,'FaceColor','k');b
%     hold on
% end
% title('无人机覆盖范围')
% xlabel('x')
...
%     for i = 1:UAVnumber
%         set(CoverageRangeHandle(i),'Position',[PathX(counter+1,i)-...
%             coverageRadius PathY(counter+1,i)-coverageRadius coverageRadius*2 ...
%             coverageRadius*2],'Curvature',[1,1]);
...
%             [vector,value]=eig(L);
% 
%         NewRang(k,m) = value(2,2);
%     end
% end
% figure(6)
% 
% for k=1:4
%  plot(R-0.1:-0.1:0,NewRang(k,:));
%  hold on;
% end
% plot(6.4,0,'o','LineWidth',5,'color','r')
